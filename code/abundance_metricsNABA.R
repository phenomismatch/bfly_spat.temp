...
naba.circles<-read.csv("C:/Users/eal109/Documents/Git/data_raw/NABA/naba_circles.csv") 
...
x1<-obs.data %>%
  group_by(group, ScientificName, CommonName) %>%
  summarize(count.recs=n(), mean.abund=mean(NumSeen, na.rm=T)) %>%
  arrange(group, desc(count.recs))
...
## 2022-05 data summary

tax.summary<-naba.sp.abundance %>%
  mutate(n=1) %>%
